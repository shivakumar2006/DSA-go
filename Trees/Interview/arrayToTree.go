...
func (bst *BinarySearchTree) display(node *Node, details string) {
	...
	bst.display(node.left, fmt.Sprintf("the left child of %d : ", node.data))
	bst.display(node.right, fmt.Sprintf("the right chils of the %d: ", node.data))
}

func (bst *BinarySearchTree) DisplayTree(node *Node, level int) {
	if node == nil {
		return
	}

	bst.DisplayTree(node.right, level+1)
	...
	bst.DisplayTree(node.left, level+1)
}
